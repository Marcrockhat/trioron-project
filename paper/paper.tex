...
\subsection{The v1.1 Working Recipe: Credit, Manifold, and Cosine Head}
\label{sec:methods-v11}

The v1.0 substrate described above couples EWC anchoring with
growth, dreaming, and manifold replay. The v1.1 recipe replaces
the EWC penalty with a strictly stronger anchoring mechanism and
makes two additional changes to the head and replay path that
together constitute the working deployment configuration. The
three load-bearing components are:

\paragraph{Credit-based row freezing.}
Per-cell \emph{engagement} is tracked as the mean activation rate
of each node over a training phase. Cells whose engagement exceeds
a threshold (default $0.3$) earn \emph{credit}: their gradients
are zeroed on all subsequent tasks. This replaces the quadratic
EWC penalty with a strictly stronger anchor --- zero gradient
rather than a soft pull toward the anchor --- at cell granularity.
Credited rows are effectively frozen in place; new tasks can only
learn through uncredited (low-engagement) cells and any newly
grown nodes.

\paragraph{Manifold replay with detached L1.}
The manifold-replay mechanism of \S\ref{sec:methods-manifold} is
modified so that synthetic codes sampled from the per-class
$(\mu_c, \sigma_c)$ sketch are forwarded through L1 under
\texttt{torch.no\_grad()} (L1 detached). The resulting features
are then passed to the head under cross-entropy loss with
gradients enabled only on the head. This ensures that replay
updates the head's decision boundary for past classes without
disturbing credited rows in L1, preserving the credit-based
anchoring.

\paragraph{Cosine head.}
The output head computes logits as
\begin{equation}
  \text{logits}_{b,k} = \tau \cdot \cos\!\bigl(f_{b},\, W_k\bigr),
  \qquad \tau = 16,
\end{equation}
where $f_b$ is the L1 feature vector for sample $b$, $W_k$ is the
$k$-th head weight row, and $\cos(\cdot, \cdot)$ is the cosine
similarity. The head carries no bias term. By bounding output
magnitudes to $[-\tau, \tau]$, the cosine head dampens the bias
accumulation across tasks that a standard linear head exhibits
when new class columns are added lazily during class-incremental
learning.

\paragraph{Relationship to v1.0.}
The v1.1 recipe is an incremental refinement, not a new
architecture: the substrate, growth trigger, dreaming phase,
manifold storage, and multi-branch absorption are unchanged. The
three changes --- credit for EWC, L1-detached replay, cosine head
--- address specific failure modes observed in the v1.0 deployment
(head-bias drift, replay-induced credit violation, unbounded logit
magnitudes) rather than adding new structural capacity. Post-trim
benchmark results comparing v1.1 arms against the v1.0 headline
are reported in \S\ref{sec:results-v11}.
...
\S\ref{sec:results-v11} reports the v1.1 post-trim ablation
isolating credit-based row freezing and manifold replay.
...
\subsection{v1.1 Post-Trim Ablation}
\label{sec:results-v11}

Table~\ref{tab:v11-posttrim} reports the v1.1 arms on chained-15
at $n = 3$, 8 epochs per task, isolating the contributions of
credit-based row freezing and manifold replay with detached L1
under the cosine head.

\begin{table}[h]
  \centering
  \caption{chained-15, $n = 3$, 8 epochs/task --- v1.1 post-trim
           ablation. The v1.0 paper headline ($n = 10$, 4 epochs/task)
           is included for reference.}
  \label{tab:v11-posttrim}
  \begin{tabular}{lcc}
    \toprule
    arm                              & task-aware            & full              \\
    \midrule
    \arm{AXIS6\_credit\_manifold}    & $0.959 \pm 0.001$     & $0.563 \pm 0.004$ \\
    \arm{AXIS6\_credit} (no manifold)& $0.971 \pm 0.009$     & $0.168 \pm 0.017$ \\
    \arm{AXIS6\_emerge} (no credit)  & $0.735 \pm 0.052$     & $0.073 \pm 0.013$ \\
    \arm{BASELINE\_frozen}           & $0.657 \pm 0.043$     & $0.041 \pm 0.006$ \\
    \midrule
    v1.0 headline ($n = 10$)         & $0.961$               & $0.601$           \\
    \bottomrule
  \end{tabular}
\end{table}

\paragraph{What the table shows.}
Credit-based row freezing is load-bearing: removing it
(\arm{AXIS6\_emerge}) collapses task-aware from $0.959$ to
$0.735$ and full-softmax from $0.563$ to $0.073$. Without
credit, unconstrained gradient flow through L1 overwrites
prior-task features during subsequent training phases.
Manifold replay is load-bearing for full-softmax but not for
task-aware: the credit-only arm (\arm{AXIS6\_credit}) retains
$0.971$ task-aware but loses $0.395$ absolute on full-softmax
compared to the credit+manifold arm, confirming that manifold
replay's role is head calibration across the growing class set,
not feature preservation. The frozen baseline confirms that
credit and manifold each contribute independently above the
no-learning floor. The v1.0 headline at $n = 10$ and 4
epochs/task is not directly comparable (different epoch count,
different seed cohort size) but is included as a reference point;
the v1.1 credit+manifold arm matches it on task-aware ($0.959$
vs $0.961$) while trailing on full-softmax ($0.563$ vs $0.601$),
a gap attributable to the epoch-count difference and the stricter
$n = 3$ seed budget.
