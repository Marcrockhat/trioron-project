...
during training, with cross-layer fan-in/fan-out consistency. See
Appendix~\ref{app:trioron-node} and \ref{app:structural-plasticity}
for the full forward pass, per-node EWC derivation, and growth/prune
machinery.
...
on a shared-L0 invariant; alternative protocols and the closed-form
translator that handles donors trained at distinct random seeds are
documented in Appendix~\ref{app:l0-handshake}.
...
The paired margin of \arm{fixed\_ewc\_small} versus
\arm{grown\_uncapped\_dream} is $-0.023$ absolute on full-softmax at
$n = 10$ ($-1.79\sigma$), $-0.024$ on domain-aware ($-2.55\sigma$),
and $-0.012$ on task-aware ($-4.63\sigma$). The task-aware effect is
the most reliable: per-arm seed-to-seed standard deviation on
task-aware sits at $0.001$--$0.004$, so even the small absolute
$\Delta$ registers as a tight $\sigma$. Earlier $n = 3$ headline
numbers had read $-0.018$ full ($-6.36\sigma$) and $-0.009$ task
($-4.08\sigma$); the additional seven seeds tightened the absolute
$\Delta$ slightly while widening the cross-seed std on full-softmax,
which is the honest summary --- some seeds underperformed the
original three-seed mean. We report this as a defensible win on
task-aware and a within-noise difference on full-softmax.
...
The honest reading at $n = 10$ is: \emph{on full-softmax and
domain-aware, manifold-grown is consistently above PackNet, HAT,
Online EWC, and LwF in absolute terms, by margins between $0.32$ and
$0.55$ on class-incremental chained-15.} On task-aware, the field
collapses to a tight cluster near $0.95$, paired $\sigma$ values are
modest, and the LwF tie ($-0.003$, $-1.3\sigma$) is now within
seed noise rather than the $-26\sigma$ artifact the original $n = 3$
table reported.
