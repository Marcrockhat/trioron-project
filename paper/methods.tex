...
manifold replay (\S\ref{sec:manifold}). The branches share the same
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item Compute branch gates by softmax over branches:
        ...
        where $T$ is a temperature ($T = 1$ default). Rocky's bet that
        bleed adds new function rather than destroys it is licensed
        here: multiple branches can co-fire.
...
\emph{Expert-Gate}-style router (Aljundi et al.\ 2017) using the
