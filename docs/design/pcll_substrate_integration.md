...
| D10 | I4 scope (Rocky, s030 discussion): dynamic-perception manifold. (a) signatures indexed by CELL ID with pad/remap; (b) per-DIMENSION counts m_f (scalar m freezes new dims); (c) per-class SHADOW accumulators = the default extension path (recruit-without-relearn generalised; new dims warm up to the n≥K² testimony floor); (d) FRAME REGISTRY + translation layer: pocket coordinates are unstable (gain frame moves on receptor attach; labeled lines move on census-k revision) → signatures carry frame stamps, translated closed-form on (center,width) when the frame changes (continuous: statistical affine from tracked E[lo],E[hi]; discrete: exact line remap), updated each manifold cycle; (e) ship/wake serialises signatures WITH frame stamps. Gate addition: resolution on old classes survives a receptor attach (frame shift). | **approved direction — to build in I4** |
