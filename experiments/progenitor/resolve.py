"""One-cycle resolution — graded output + margins over the √N floor (design §5).

Per-class evidence is a matched filter in phasor space. A class's range schedule
(feed.py) gives a per-feature template phasor

    T_cf = mean of e^{iθ(q)} over the range's INTERIOR quanta

— a point range has |T|=1, a wide (non-discriminative) range self-down-weights,
and reference pockets (q=0/1000) contribute nothing, mirroring evidence_mask.
Evidence against the lock-in accumulator A (complex, per feature):

    E_c = Σ_f Re(A_f · conj(T_cf))

Under noise A_f is a 2-D random walk (per-component variance n_f/2), so E_c is
exactly Gaussian with var σ_c² = Σ_f |T_cf|²·n_f/2 — both resolution tests are
parameter-free √-statistics (design §5):

  EMPTY      — no class clears K·σ_c: nothing in the stream (terminal AND stress, §6)
  FRUSTRATED — evidence clears the floor but the top-two gap sits inside ITS noise
               band, σ_gap² = Σ_f |T_top,f − T_2nd,f|²·n_f/2 (ambiguity-stress)
  RESOLVED   — the top class clearly leads (gap margin > K)

The output is graded: the full ranking of per-class margins ("more likely chicken
than dog"), never a one-hot. Shared evidence (overlapping ranges) lowers the gap
margin — measured honestly, not an error.
"""
...
from feed import Schedule
from lockin import N_QUANTA, LockIn
...
class Resolver:
    ...
    def resolve(self, li: LockIn, k_abs: float = 3.0, k_gap: float = 3.0) -> Resolution:
