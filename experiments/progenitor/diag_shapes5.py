...
report(f"grouping per-group read (106+205){' CANON-'+CANON if CANON else ''}", pred)
...
for sp, yy in (("test_fresh", yfr), ("test_held", yho), ("test_stress", yst)):
    ...
    if sp == "test_stress":
        ...
        print("  single-object shape leaf on grouped 106" + (" CANON-"+CANON if CANON else "") + ": " + " | ".join(f"{k} {float((pr[m]==yy['y_shape'][m]).float().mean()):.3f}" for k, m in sl.items()), flush=True)
    else: print(f"  single-object shape leaf on grouped 106{' CANON-'+CANON if CANON else ''}: {sp} {float((pr==yy['y_shape']).float().mean()):.3f}", flush=True)
