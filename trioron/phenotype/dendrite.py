...
partition, passes it through a per-branch quadratic nonlinearity σ(z) = z + z²,
and the branch outputs are pooled at the soma with learned per-branch weights α:

    y_v = b_v + Σ_k α_{v,k} · σ( Σ_{u ∈ B_k(v)} w_{vu}·a_u ),  σ(z) = z + z²

The ``z²`` term supplies curvature/products, so a single branch can carve a
relational boundary (a lone quad unit solves XOR) — well suited to the multimodal
class regions of the capacity-hard taxonomy, where a class occupies several
disjoint modes. Because the partition is per-branch, different branches can
specialize to *disjoint* regions of the fan-in.

**K = 1 is byte-identical to linear** (spec §3.6 back-compat guarantee): with a
single branch the quad term is suppressed and α₀ = 1, so y = b + Σ w·a. The
